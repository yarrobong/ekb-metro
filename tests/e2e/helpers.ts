...
export const TEST_TIME_LAST_TRAIN = "2024-01-06T00:14:40+05:00";
...
export async function selectDestination(page: Page, destinationName: string) {
  await page.getByRole("button", { name: "Выбрать станцию" }).click();
  await expect(
    page.getByRole("heading", { name: "Выберите станцию назначения" }),
  ).toBeVisible();
  await page.getByRole("button", { name: new RegExp(destinationName, "i") }).click();
}
